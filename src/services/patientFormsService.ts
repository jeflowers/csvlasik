...
export async function submitMedicalHistory(
  ...
  try {
    ...
    const sanitizedData = {
      ...
      user_id: userId,
      current_eye_conditions: sanitizeInput(data.currentEyeConditions),
      previous_eye_surgeries: sanitizeInput(data.previousEyeSurgeries),
      current_medications: sanitizeInput(data.currentMedications),
      drug_allergies: sanitizeInput(data.drugAllergies),
      has_diabetes: data.hasDiabetes,
      family_history_eye_disease: data.familyHistoryEyeDisease,
      status: 'submitted',
    };
