...
const LanguageSelector: React.FC<LanguageSelectorProps> = ({ variant = 'dark' }) => {
  ...
  const dropdownPanel = isOpen ? createPortal(
    <div
      ref={panelRef}
      className="min-w-[280px] bg-white rounded-lg shadow-xl border border-gray-200 max-h-[500px] overflow-y-auto"
      style={{
        position: 'fixed',
        top: panelPos.top,
        right: isRTL ? undefined : panelPos.right,
        left: isRTL ? panelPos.right : undefined,
        zIndex: 99999,
      }}
    >
