        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h2 className="text-3xl lg:text-4xl font-serif chopard-text-primary mb-4">
              {t('common:video.watchProcedure', 'Watch the LASIK Procedure')}
            </h2>
            <p className="text-xl chopard-text-secondary max-w-3xl mx-auto font-light">
              {t('common:video.description', "See Dr. Flowers' revolutionary LASIK technique in action")}
            </p>
          </div>
      ...
      <section className="py-16 chopard-hero">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[
              { icon: <Eye className="h-8 w-8 chopard-text-accent" />, key: 'clarity' },
              { icon: <Clock className="h-8 w-8 chopard-text-accent" />, key: 'quick' },
              { icon: <Shield className="h-8 w-8 chopard-text-accent" />, key: 'safety' },
              { icon: <Users className="h-8 w-8 chopard-text-accent" />, key: 'satisfaction' },
              { icon: <Star className="h-8 w-8 chopard-text-accent" />, key: 'lasting' },
              { icon: <ArrowRight className="h-8 w-8 chopard-text-accent" />, key: 'lifestyle' }
            ].map((benefit, index) => (
              <div key={index} className="chopard-card p-8 rounded-xl text-center">
                <div className="mb-4">{benefit.icon}</div>
                <h3 className="text-xl font-serif chopard-text-primary mb-3">{t(`lasik.benefits.items.${benefit.key}.title`)}</h3>
                <p className="chopard-text-secondary font-light">{t(`lasik.benefits.items.${benefit.key}.description`)}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-16 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="space-y-12">
            {[
              { step: '1', key: 'examination', image: '/assets/images/procedures/lasik/process/step-01-examination.png' },
              { step: '2', key: 'flap', image: '/assets/images/procedures/lasik/process/step-02-flap-creation.png' },
              { step: '3', key: 'correction', image: '/assets/images/procedures/lasik/process/step-03-laser-reshaping.png' },
              { step: '4', key: 'recovery', image: '/assets/images/procedures/lasik/process/step-04-recovery.png' }
            ].map((step, index) => (
              <div key={index} className={`grid grid-cols-1 lg:grid-cols-2 gap-12 items-center ${index % 2 === 1 ? 'lg:grid-flow-col-dense' : ''}`}>
                <div className={index % 2 === 1 ? 'lg:col-start-2' : ''}>
                  <div className="flex items-center mb-6">
                    ...
                    <h3 className="text-2xl font-serif chopard-text-primary">{t(`lasik.process.steps.${step.key}.title`)}</h3>
                  </div>
                  <p className="text-lg chopard-text-secondary font-light">{t(`lasik.process.steps.${step.key}.description`)}</p>
                </div>
                <div className={index % 2 === 1 ? 'lg:col-start-1 lg:row-start-1' : ''}>
                  <img
                    src={step.image}
                    alt={`LASIK Step ${step.step}: ${t(`lasik.process.steps.${step.key}.title`)}`}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            <div className="chopard-card p-8 rounded-xl">
              ...
              <ul className="space-y-4">
                {t('lasik.candidacy.ideal.items', { returnObjects: true }).map((item, index) => (
            ...
            <div className="chopard-card p-8 rounded-xl">
              ...
              <ul className="space-y-4">
                {t('lasik.candidacy.notSuitable.items', { returnObjects: true }).map((item, index) => (
                  <li key={index} className="flex items-start">
          ...
          <div className="text-center mt-12">
            ...
            <Link
              ...
              className="inline-flex items-center chopard-button px-8 py-3 rounded-lg transition-all duration-300"
            >
              {t('common:cta.scheduleEvaluation', 'Schedule Free Candidacy Evaluation')}
              <ArrowRight className="ml-2 h-5 w-5" />
            </Link>
          </div>
        </div>
